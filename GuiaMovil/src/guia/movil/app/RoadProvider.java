...
public class RoadProvider {
        ...
        public static String getUrlR(double fromLat, double fromLon, double toLat,
                double toLon) {// connect to map web service
        StringBuffer urlString = new StringBuffer();
...
class KMLHandler extends DefaultHandler {
        ...
        public Pointt[] addPoint(Pointt[] points) {
                Pointt[] result = new Pointt[points.length + 1];
                for (int i = 0; i < points.length; i++)
                        result[i] = points[i];
                result[points.length] = new Pointt();
                return result;
        }
